...
namespace ImageClassifierSpatialPooler
{
    class Binarizer
    {
        ...
        public Binarizer(int redThreshold = 230, int greenThreshold = 230, int blueThreshold = 230, int targetWidth = 0, int targetHeight = 0)
        {
        ...
        public void CreateBinary(string image, string binaryFile)
        {
            ...
            using (StreamWriter writer = File.CreateText(binaryFile))
            {
                if (this.m_TargetSize != null)
                    img = new Bitmap(img, this.m_TargetSize.Value);
